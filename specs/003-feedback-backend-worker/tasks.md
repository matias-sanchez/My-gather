...
- [x] T011 [P] [US3] Implement `feedback-worker/src/ratelimit.ts`: fixed UTC-hour window keyed `rl:<ipHash>:<UTC-hour>:<random>` (unique key per request, same-prefix decisions serialized inside the active Worker isolate, counted via `KV.list({prefix}).keys.length`). App ID is the salt. Returns `{allowed, retryAfterSeconds}`.
...
- [x] T029 `cd feedback-worker && npm install && npm test && npm run deploy` — shipped as commit 636a332. Worker URL recorded. Smoke test issues #25 and #26 indicate normal free-tier operation with no CPU-exhaustion errors over several days. If a future change pushes cpuTime above 9 ms median, the research R9 mitigation ladder (KV-cached installation token, then paid tier) still applies.
