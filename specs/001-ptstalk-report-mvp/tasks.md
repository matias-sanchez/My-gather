...
- [x] T058 [P] [US3] `render/variables_test.go::TestVariablesSearchMarkup`: assert the rendered Variables subview carries `<input type="search">` with a binding hook (class `variables-search` + `data-snapshot`, or equivalent — the shipped implementation uses the class + data-snapshot pair rather than an `id`; an `id` is accepted for a future refactor), and that every data row carries a `data-variable-name` attribute for the client-side filter (FR-013).
...
- **Phase 6 US4 tests**: all six complete. T063 (innodbstatus), T064 (mysqladmin golden), T065 (pt-mext), T066 (snapshot-boundary reset), T067 (variable drift), T068 (processlist parser-side) plus T069 (DB render golden) and T070 (mysqladmin toggle markup) are now `[x]`.
