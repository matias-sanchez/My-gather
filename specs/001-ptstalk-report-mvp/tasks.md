...
- [x] T019 Commit anonymised fixtures under `testdata/example1/` and `testdata/example2/` produced by `scripts/anonymise-fixtures.sh`. Both fixtures are now checked in: `testdata/example2/` (16 files) and `testdata/example1/` (17 files — adds `pt-summary.out` / `pt-mysql-summary.out` alongside the seven source-file suffixes for discovery-test coverage).
...
- [x] T080 [P] `tests/coverage/suffix_golden_test.go::TestEverySuffixHasGolden` strengthens T021's fixture-only check by asserting every `model.Suffix` in `KnownSuffixes` also has at least one golden under `testdata/golden/`. Accepts either a parser JSON golden whose filename starts with the suffix (or the digit-stripped equivalent for `innodbstatus1` → `innodbstatus.`) OR a render-section HTML golden bundle (`os.example*.html`, `db.example*.html`, `variables.example*.html`) whose membership is declared in an explicit `renderBundles` map. A future refactor that merges goldens into a new bundle shape only has to update the map.
...
- [x] T089 [P] `parse/partial_recovery_test.go::TestPartialRecoveryAllParsers` (F28 — FR-008 gap) parametrises over the six collectors without an existing partial-recovery test (top, vmstat, variables, innodbstatus, mysqladmin, processlist). For each: reads the committed fixture, cuts at a mid-line position inside the first 25 % of the file, re-parses, and asserts the parser returns non-nil data with non-zero elements (Principle III's core "graceful recovery" invariant). If Warning/Error diagnostics are emitted their metadata must satisfy FR-008 (non-empty Location, correct SourceFile). The "partial must be strictly smaller than full" invariant was tried first but proved fixture-dependent for scalar / summary-heavy collectors; dropped in favour of the non-nil/non-zero formulation. A real FR-008 gap in `parse/mysqladmin.go`'s trailing-drift warning (empty Location) surfaced from this test and was fixed in the same PR by adding `Location: "column <len(vs)>"`.
- [x] T090 [P] `parse/version_test.go::TestDetectFormat` (F29 — FR-024 gap) drives both halves of the detector contract: 7 positive subtests (one per Suffix, each feeding a representative V1 header byte sequence; assert FormatV1) and 7 negative subtests (bytes that lack the V1 signature; assert FormatUnknown per FR-024's "prefer Unknown over misclassification"), plus a default-case subtest for an unknown Suffix. Shipped v1 detector only discriminates V1-vs-Unknown; V2 detection is deferred per `parse/version.go:43-48` and the test note says to add V2 subtests when parsers learn to branch on V2 — keeps the test aligned with the actually-shipped contract rather than an aspirational 14-subtest plan.
...
A mechanical pass compared every task ID against the real repository state for the full current range, **T001–T119 (119 tasks total)**. `84 / 119 tasks (≈71 %) carry an [x] mark` on this branch. The pipeline builds, `go test ./...` is green, and the binary produces a deterministic three-section report against both `testdata/example1/` and `testdata/example2/`. The 2026-04-22 fixtures + coverage bundle E closes T019 (example1 anonymised fixtures now committed alongside example2), T080 (`TestEverySuffixHasGolden` — strengthens T021 with a per-suffix golden check accepting either parser JSON or render bundle HTML), T089 (`TestPartialRecoveryAllParsers` — six-parser parametrised partial-recovery harness; surfaced and fixed a real FR-008 gap where `parseMysqladmin`'s trailing-drift Warning emitted an empty Location), and T090 (`TestDetectFormat` — 15 subtests covering the positive V1 detection and negative Unknown fallback for every Suffix). Two parallel bundles (C and D) are in flight and, once merged and rebased, bring the totals to 98 / 119 (PR C's FR-033..037 feature tests) and then 98 / 119 → 98 already; PR D's 13 CLI+integration task flips land separately. The 2026-04-22 render-goldens bundle B closed T046 (OS section golden), T057 (Variables section golden), T058 (Variables search markup), T069 (DB section golden), T070 (mysqladmin toggle markup — asserts against the JSON payload since the shipped toggle is JS-built, not server-rendered), and T096 (OS subview anchors / `sub-os-{iostat,top,vmstat}` reachable from the nav rail); `testdata/golden/` now also holds `os.example2.html`, `variables.example2.html`, `db.example2.html`. The earlier 2026-04-22 parser-goldens bundle A closed T044 (top), T045 (vmstat), T063 (innodbstatus), T064 (mysqladmin golden), T066 (mysqladmin snapshot-boundary reset per FR-030), and T067 (mysqladmin variable drift per R8.C), bringing `testdata/golden/` to seven committed parser goldens covering every per-collector parser output. The 2026-04-22 spec-drift reconciliation added five shipping features captured as FR-033–FR-037 (MySQL-defaults badging, mysqladmin category chooser, default-hidden initial-tally column, `Cmd/Ctrl+\` nav shortcut, `@media print` stylesheet); their implementation tasks (T097, T099, T101, T103, T105) are already live in `render/assets/` and `render/render.go` and carry `[x]` marks, while their paired tests (T098, T100, T102, T104, T106) land with PR C currently in flight. The 2026-04-22 boundary-markers PR closed T054 (multi-Snapshot concatenation + dashed vertical markers on every time-series chart per FR-018 / FR-030). The 2026-04-22 UX-quality reconciliation added FR-038–FR-041 + SC-011 (research R13) with a committed audit checklist (T107 `[x]`) and Phase 8 release-gate tasks T108–T119 `[ ]`, including a shared-stylesheet refactor (T108) that extracts typography / spacing / palette / chart invariants into named CSS custom properties. What remains across the whole feature is **performance + memory + a11y + determinism-stress tests** (T078, T079, T081, T082), **the UX audit pass itself**, plus a handful of documentation items (T085) and the CSS refactor (T108, T109) — no core implementation is unshipped.
