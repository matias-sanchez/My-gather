...
- [x] **TPM-1** Add the `Environment` top-level section (commit `0790e17`, PR #13). New `EnvironmentSection` type in `model/sections.go`, new `render/environment.go` + `render/templates/environment.html.tmpl`, fixture wiring in `testdata/example2/`. Renders first per FR-005's revised order. Surfaces host, kernel, CPU, memory, MySQL version, data-dir, key runtime variables — the highest-signal slice when an engineer opens a fresh capture (Principle XI).
