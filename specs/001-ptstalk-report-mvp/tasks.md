...
- [x] T030 [P] [US1] `cmd/my-gather/main_test.go::TestOutputInsideInputRejected`: invokes `run()` with `-o <somewhere-inside-input-dir>`; asserts exit code 7 (exitOutputInsideInput) and the report file was NOT written (FR-029).
- [x] T031 [US1] `cmd/my-gather/main_test.go::TestOverwriteGuard`: invokes `run()` twice without `--overwrite`; asserts exit code 6 (exitOutputExists) on the second call. **Sequential with T030** (same file).
- [x] T032 [P] [US1] `tests/integration/e2e_test.go::TestE2EExample2`: runs the compiled binary against `testdata/example2/`; asserts exit 0, output file exists with non-zero size, rendered HTML carries `id="sec-os"`, `id="sec-variables"`, `id="sec-db"`, and the `Verbatim passthrough:` banner (FR-005 / FR-020 / FR-026).
- [x] T033 [P] [US1] `tests/integration/offline_test.go::TestOfflineRender`: scans the rendered HTML for six attribute-scoped network probes (`src="http://`, `src="https://`, `src="//`, same three for `href=`), two CSS `url(…)` probes accepting only `data:` / file-relative paths, a `<script … src=…>` probe, and a `<link rel="stylesheet">` probe. Complementary to the existing `tests/lint` package which enforces offline at the Go source level (FR-022 / Principle IX).
- [x] T034 [P] [US1] `render/app_test.go::TestCollapsePersistenceRoundTrip`: source-level assertion on `render/assets/app.js` — a real browser-level harness is rejected per Principle X. Pins six tokens (`storageGet`/`storageSet` helpers, `REPORT_ID` symbol, `collapseKey` function, `REPORT_ID` inside its body, namespace key template `"mygather:"+REPORT_ID` or `REPORT_ID+":collapse:"`), plus the save path `storageSet(collapseKey(…), …)` and the load path `storageGet(collapseKey(…))` with a `.open` reference within 400 chars (SC-010).
- [x] T035 [P] [US1] `render/nav_test.go::TestNavCoverage`: full parse.Discover + render against `testdata/example2/`; extracts the `<nav class="index">` block; asserts every `href="#…"` in the nav has a matching `id="…"` somewhere in the document and no duplicate hrefs. Accepts more `id=` anchors than nav entries (sub-subviews etc.); the spec invariant is forward-only (every nav entry resolves) (SC-009).
...
- [x] T086 `cmd/my-gather/main_test.go::TestOutputParentMissing` — asserts exit code 3 (exitInputPath) when `--out <nonexistent-dir>/...` is passed; no auto-mkdir, no output file created. Shipped implementation in `cmd/my-gather/main.go` now stats the output parent directory after the FR-029 check and returns exit 3 with a clear stderr message if it doesn't exist (F8 resolution).
...
- [x] T088 [P] `cmd/my-gather/main_test.go::TestOutputFileSingletonNoSideCar` (F27 — FR-002 gap): walks the output parent directory with `os.ReadDir` before and after a successful `run()`; diffs the entry sets; asserts exactly one new entry (the report path) and no `.tmp`, `.bak`, `.part`, or lock-file sidecars survive. Single-file-output invariant enforced.
...
- [x] T091 `cmd/my-gather/main_test.go::TestStderrSilentOnSuccess` (F30 — FR-027 gap, 1 of 3): asserts stderr is empty after a clean `run()` against `testdata/example2/` at default verbosity.
- [x] T092 `cmd/my-gather/main_test.go::TestStderrWarningMirrored` (F30 — FR-027 gap, 2 of 3): truncates the iostat source file to 50 % of its original length in a fixture copy, runs `run()`, asserts stderr contains a line matching `^\[warning\]\s+\S+:\s+.+$` per `contracts/cli.md`. Iostat targeted because its parser reliably emits `Diagnostic(Severity=Warning)` on a truncated header; mysqladmin can short-circuit early on some truncation shapes.
- [x] T093 `cmd/my-gather/main_test.go::TestStderrVerboseProgress` (F30 — FR-027 gap, 3 of 3): runs `run()` with `-v`; asserts stderr contains `[parse] …`, `[render] writing …`, and `[done] … bytes written in …s` lines in that exact order. Confirms SeverityInfo diagnostics do NOT appear on stderr at default verbosity (F13 behavior — the test scans stderr from T091's default-verbosity run).
- [x] T094 [P] `tests/integration/degraded_test.go::TestOneMissingCollector` (F31 — SC-004 gap): seven subtests (one per suffix). For each: copies `testdata/example2/` to a tempdir, deletes every file ending in `-<suffix>` (both snapshots), runs the CLI, asserts exit 0, the three top-level section anchors (`sec-os`, `sec-variables`, `sec-db`) still exist, and a banner referencing `<code>-<suffix></code>` and `Data not available` appears in the rendered HTML.
- [x] T095 `cmd/my-gather/main_test.go::TestVersionOutput` (FR-023 coverage): invokes `run("--version")`; asserts stdout contains the three semver / commit / build-date tokens emitted by `cmd/my-gather/main.go::printVersion`. ldflags-injected-at-build-time tokens are exercised by the go-test-time `version` / `commit` / `buildDate` vars.
...
A mechanical pass compared every task ID against the real repository state for the full current range, **T001–T119 (119 tasks total)**. `93 / 119 tasks (≈78 %) carry an [x] mark` on this branch. The pipeline builds, `go test ./...` is green, and the binary produces a deterministic three-section report against `testdata/example2/`. The 2026-04-22 CLI + integration bundle D closes T030 (`TestOutputInsideInputRejected` — exit 7 for FR-029), T031 (`TestOverwriteGuard` — exit 6), T032 (`TestE2EExample2` — end-to-end CLI against example2), T033 (`TestOfflineRender` — 10 network-URL probes against the rendered HTML; complements the source-level forbidden-import linter at FR-022), T034 (`TestCollapsePersistenceRoundTrip` — 6-token source-level assertion on `app.js` for SC-010; JS harness rejected per Principle X), T035 (`TestNavCoverage` — every nav `href="#…"` resolves to an `id=` target), T086 (`TestOutputParentMissing` — exit 3 for missing parent dir; shipped F8 resolution extends `cmd/my-gather/main.go` to stat the output parent), T088 (`TestOutputFileSingletonNoSideCar` — before/after `os.ReadDir` diff confirms no sidecars), T091 / T092 / T093 (stderr contract: silent on success / warning mirror / verbose progress with `[parse]` / `[render]` / `[done]` order), T094 (`TestOneMissingCollector` — 7 subtests, one per suffix, exit 0 + banner), and T095 (`TestVersionOutput` — five-line version-stamp format). A new `tests/integration/` package hosts the cross-cutting tests (`e2e_test.go`, `offline_test.go`, `degraded_test.go`) and a shared `bin_test.go` `TestMain` that builds the binary once per package invocation. The 2026-04-22 render-goldens bundle B closed T046 (OS section golden), T057 (Variables section golden), T058 (Variables search markup), T069 (DB section golden), T070 (mysqladmin toggle markup — asserts against the JSON payload since the shipped toggle is JS-built, not server-rendered), and T096 (OS subview anchors / `sub-os-{iostat,top,vmstat}` reachable from the nav rail); `testdata/golden/` now also holds `os.example2.html`, `variables.example2.html`, `db.example2.html`. The earlier 2026-04-22 parser-goldens bundle A closed T044 (top), T045 (vmstat), T063 (innodbstatus), T064 (mysqladmin golden), T066 (mysqladmin snapshot-boundary reset per FR-030), and T067 (mysqladmin variable drift per R8.C), bringing `testdata/golden/` to seven committed parser goldens covering every per-collector parser output. The 2026-04-22 spec-drift reconciliation added five shipping features captured as FR-033–FR-037 (MySQL-defaults badging, mysqladmin category chooser, default-hidden initial-tally column, `Cmd/Ctrl+\` nav shortcut, `@media print` stylesheet); their implementation tasks (T097, T099, T101, T103, T105) are already live in `render/assets/` and `render/render.go` and carry `[x]` marks. A parallel feature-tests bundle C in flight closes their paired tests (T098, T100, T102, T104, T106); once that merges this branch rebases to 98 / 119. The 2026-04-22 boundary-markers PR closed T054 (multi-Snapshot concatenation + dashed vertical markers on every time-series chart per FR-018 / FR-030). The 2026-04-22 UX-quality reconciliation added FR-038–FR-041 + SC-011 (research R13) with a committed audit checklist (T107 `[x]`) and Phase 8 release-gate tasks T108–T119 `[ ]`, including a shared-stylesheet refactor (T108) that extracts typography / spacing / palette / chart invariants into named CSS custom properties. What remains across the whole feature is **example1 fixtures (T019)**, **the remaining test gaps** (performance, memory, a11y, determinism-stress, extra partial recovery, version detection, suffix coverage metatest), **the UX audit pass itself**, plus a handful of documentation and coverage gaps — no core implementation is unshipped.
...
### What's still open (26 tasks on this branch; 21 after PR C merges)

**Test gaps** (biggest bucket):

- US1 CLI + integration + JS-side tests: complete. T030, T031, T032, T033, T034, T035 are all `[x]` as of bundle D.
- US2 render golden: complete. T046 is `[x]` (OS section golden).
- US3 variables render golden + search markup: complete. T057, T058 are `[x]`.
- US4 DB render goldens + markup: complete. T069, T070 are `[x]`.
- Polish tests still pending: T078 (perf), T079 (memory), T080 (suffix golden coverage), T081 (a11y), T082 (determinism stress), T089 (partial recovery across parsers), T090 (version detection). T086 (output-parent-missing), T088 (single output file), T091 / T092 / T093 (stderr contract), T094 (degraded one-missing-collector), T095 (version ldflags) are now `[x]` as of bundle D. SC-005 OS anchors (T096) is `[x]`.
