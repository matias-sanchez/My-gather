...
- **FR-015**: If the Submit-path fallback fires (Worker unreachable per feature 003, or `window.open` returns null because the popup is blocked), the dialog MUST display a visible fallback link with the same pre-filled URL so the user can click through manually without retyping. _(Originally scoped only to the popup-blocked case; feature 003 widens the trigger to any Worker failure mode that hits the `window.open` fallback path — see `specs/003-feedback-backend-worker/contracts/ui.md` §"Fallback".)_
