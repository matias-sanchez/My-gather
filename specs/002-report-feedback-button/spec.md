...
- **Superseded by 003**: the Submit destination, the GitHub category vs. label mapping, the Submit-time URL construction, the success / error / throttle / fallback states. Read `specs/003-feedback-backend-worker/contracts/api.md` and `specs/003-feedback-backend-worker/contracts/ui.md` for these.
...
- **FR-014**: The dialog MUST display a visible reminder that submitted content becomes public, positioned where the user sees it before clicking Submit. _(Originally specified as "public GitHub Discussion"; superseded by feature 003 — the Submit happy path posts a public GitHub **Issue** instead. The reminder text rendered on main reads "Posts a new issue to GitHub.")_
- **FR-015**: If `window.open` returns null (popup blocked) when the Submit-path falls through to `doLegacyFallback` (`render/assets/app.js`), the dialog MUST display a visible fallback link with the same pre-filled URL so the user can click through manually without retyping. _(Trigger condition is unchanged from the 002 baseline — the fallback link surfaces only on popup-blocked. Feature 003 widens the set of UPSTREAM callers that reach `doLegacyFallback` (any Worker 5xx / network error / timeout, not just an initial Submit click), but each call still attempts `window.open` first and only unhides the fallback link if the popup is blocked. See `specs/003-feedback-backend-worker/contracts/ui.md` §"Fallback" for the Worker→legacy decision tree.)_
